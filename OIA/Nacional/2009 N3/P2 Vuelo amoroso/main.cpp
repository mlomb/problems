...
int main(){
	...
	vector<int> pajaro_path;
	for(int col = I - 1; col >= 1; col--){
		pajaro_path.push_back(minFil);
		//cout << pajaroDP[minFil][col] << " " << minFil << endl;
		if(minFil - 1 == -1){
			minFil += 1;
			continue;
		}
		if(minFil + 1 > Y_SIZE - 1){
			minFil -= 1;
			continue;
		}
		
		if(pajaroDP[minFil + 1][col - 1] > pajaroDP[minFil - 1][col - 1])
			minFil -= 1;
		else
			minFil += 1;
	}
	
	reverse(pajaro_path.begin(), pajaro_path.end());
	
	for(int i : pajaro_path)
		fout << i << "\n";
